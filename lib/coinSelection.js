...
var CoinSelection = {
     ...
     */
    setLoader: function (lib) {
        Loader = {
            Cardano: lib
        };
    },
    setProtocolParameters: function (minUTxO, minFeeA, minFeeB, maxTxSize) {
        protocolParameters = {
            minUTxO: minUTxO,
            minFeeA: minFeeA,
            minFeeB: minFeeB,
     ...
     */
    randomImprove: function (inputs, outputs, limit) {
        ...
        var utxoSelection = {
            selection: [],
            remaining: __spreadArray([], inputs),
            subset: [],
            amount: Loader.Cardano.Value.new(Loader.Cardano.BigNum.from_str('0')),
        };
        var mergedOutputsAmounts = mergeOutputsAmounts(outputs);
        // Explode amount in an array of unique asset amount for comparison's sake
        var splitOutputsAmounts = splitAmounts(mergedOutputsAmounts);
        ...
        var maxFee = BigInt(protocolParameters.minFeeA) *
...
function randomSelect(utxoSelection, outputAmount, limit, minUTxOValue) {
    ...
    if (isQtyFulfilled(outputAmount, utxoSelection.amount, minUTxOValue, nbFreeUTxO)) {
        utxoSelection.remaining = __spreadArray(__spreadArray([], utxoSelection.remaining), utxoSelection.subset);
...
function improve(utxoSelection, outputAmount, limit, range) {
    ...
    var utxo = utxoSelection.subset
        .splice(Math.floor(Math.random() * nbFreeUTxO), 1)
        .pop();
    var newAmount = Loader.Cardano.Value.new(Loader.Cardano.BigNum.from_str('0'))
        .checked_add(utxo.output().amount())
        .checked_add(outputAmount);
    if (abs(getAmountValue(range.ideal) - getAmountValue(newAmount)) <
...
function isQtyFulfilled(outputAmount, cumulatedAmount, minUTxOValue, nbFreeUTxO) {
    ...
    if (minUTxOValue && BigInt(outputAmount.coin().to_str()) > 0) {
        var minAmount = Loader.Cardano.Value.new(Loader.Cardano.min_ada_required(cumulatedAmount, Loader.Cardano.BigNum.from_str(minUTxOValue.toString())));
 ...
 */
var cloneUTxOList = function (utxoList) {
 ...
 */
var cloneValue = function (value) { return Loader.Cardano.Value.from_bytes(value.to_bytes()); };
// Helper
function abs(big) {
